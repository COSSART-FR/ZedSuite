...
pub struct OlsInspection {
    ...
    /// Container format version (804 = WinOLS 5.84).
    pub format_version: u32,
    /// Map definitions carried by the project (see `ols_maps`): 0 when the
    /// project has none, or when its layout is not one this build can read.
    pub maps_count: usize,
    /// "hilo" / "lohi": how most of those maps store their values.
    pub byte_order: Option<String>,
}

/// Offset of the first saved version's bytes, when the container has a
/// version directory: the map records all lie before it.
pub fn version_data_start(data: &[u8]) -> Option<usize> {
    let dir = find_version_directory(data, data.len() as u64)?;
    Some(dir.version_data_start as usize)
}
...
pub fn inspect(data: &[u8]) -> Option<OlsInspection> {
    ...
    let versions = (0..dir.num_versions)
        ...
        .collect();
    let maps = crate::ols_maps::parse_maps(data, dir.version_record_size);
    let byte_order = crate::ols_maps::byte_order(&maps).map(|s| s.to_string());
